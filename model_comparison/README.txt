...
Due to different libraries used, the libraries needed for the training of the models are different and incompatible with the libraries used for the model comparison. The required libraries for the model comparison are specified in the file "requirements_tf1.15". 
...
- The modules give a framework to add further modules with that use different architectures, training data and hyperparameters. Also, threshold tuning is still an open issue. 

- Intermediate results, in particular trained models per fold, will be cached to disk. 
